...
def pieChart(xy: tuple[float, float], radius: float, baseColor: str, fillColor: str, parts: list[Part]) -> str:
    x, y = xy
    cmds: list[str] = []
    ...
    def addText(angle: float, ratioScale: float, sideScale: int, text_: str):
        cmds.append(text((x + radius*ratioScale*cos(angle) - len(text_)*0.8, y + radius*(ratioScale + sideScale*abs(sin(textAngle)*0.15))*sin(angle) - 0.8), text_))
    ...
    for part in parts:
        ...
        print(f'{ratio=}, {angle=}, {angleDeg=}, {textAngle=}, {percent=}')
...
def barChart(xy1: tuple[float, float], xy2: tuple[float, float], baseColor: str, parts: list[Part]) -> str:
    cmds: list[str] = []
    cmds.append('; Frame')
...
def makeFiguresCode() -> str:
    out: list[str] = []
    out.append(boundingBox((0, 0), (200, 200)))
    out.append('')
    out.append('')
    out.append('; Figure: bar chart')
    out.append(barChart((30, 150), (130, 190), "black", barChartParts))
    out.append('')
    out.append('')
    out.append('; Figure: pie chart')
    out.append(pieChart((75, 65), 40, "black", 'gray', piChartParts))
    out.append('')
    return '\n'.join(out)
